...
def mono_return(): return HTMLResponse("<h3>Спасибо! Проверяем платёж…</h3>")

@app.get("/payments/success")
def success(): return HTMLResponse("<h3>Оплата принята. Можно закрыть вкладку.</h3>")

@app.get("/payments/fail")
def fail(): return HTMLResponse("<h3>Оплата не прошла или отменена.</h3>")
