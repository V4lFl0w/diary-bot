...
# Канал для триала
CHANNEL_USERNAME = os.getenv("PREMIUM_CHANNEL") or getattr(settings, "premium_channel", None) or "@NoticesDiarY"
...
def _lang_of(user_row: dict | None, obj: Message | CallbackQuery | None) -> str:
    if isinstance(user_row, dict) and user_row.get("lang"):
        ...
        if isinstance(obj, Message):
            ...
            code = getattr(getattr(obj, "from_user", None), "language_code", None) or getattr(
                getattr(getattr(obj, "message", None), "from_user", None), "language_code", None
            )
...
async def _fetch_user(session: AsyncSession, tg_id: int) -> dict:
    ...
    row = (
        await session.execute(
            sql_text("SELECT id, tg_id, lang, is_premium, premium_until, tz FROM users WHERE tg_id=:tg"), {"tg": tg_id}
        )
    ).first()
    if row:
    ...
    await session.execute(
        sql_text("INSERT INTO users (tg_id, lang, is_premium) VALUES (:tg, :lang, 0)"), {"tg": tg_id, "lang": "ru"}
    )
    await session.commit()
    return {
        "id": None,
...
def _pay_kb(lang: str, tg_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text=t(lang, "btn_pay"), url=f"{base}/pay?tg_id={tg_id}")]]
    )
...
async def _grant_24h(session: AsyncSession, tg_id: int) -> None:
    ...
    await session.execute(
        sql_text("UPDATE users SET is_premium=1, premium_until=:u WHERE tg_id=:tg"), {"u": until, "tg": tg_id}
    )
    await session.commit()
