...
async def _get_or_create_user(session: AsyncSession, tg_id: int) -> User:
    ...
    user = User(tg_id=tg_id, locale=default_lang, lang=default_lang, tz=getattr(settings, "default_tz", "Europe/Kyiv"))
    session.add(user)
...
async def on_refunded_payment_stars(m: Message, session: AsyncSession) -> None:
    ...
    if user:
        ...
        payment = (
            await session.execute(select(Payment).where(Payment.external_id == rp.telegram_payment_charge_id))
        ).scalar_one_or_none()
        ...
        for tx in transactions.transactions:
            if tx.source and tx.source.type == "user" and tx.source.user.id == m.from_user.id:
                if tx.amount > 0:
                    try:
                        # 1. Возврат денег
                        await bot.refund_star_payment(user_id=m.from_user.id, telegram_payment_charge_id=tx.id)
                        ...
                        # Помечаем в БД
                        payment = (
                            await session.execute(select(Payment).where(Payment.external_id == tx.id))
                        ).scalar_one_or_none()
                        if payment:
                            payment.status = PaymentStatus.REFUNDED
                        ...
                        refunded_count += 1
                        await m.answer(
                            f"✅ Успешно вернул {tx.amount} ⭐ и аннулировал премиум!\nID транзакции: {tx.id}"
                        )
                        break
