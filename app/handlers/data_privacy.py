    ...
    try:
        row = (
            await session.execute(
                sql_text("SELECT lang FROM users WHERE tg_id=:tg LIMIT 1"),
                {"tg": tg_id},
            )
        ).first()
        if row and row[0]:
...
async def _fetch_all_data(session: AsyncSession, tg_id: int) -> dict:
    ...
    queries = {
        ...
        "journal_entries": (
            "SELECT * FROM journal_entries WHERE user_id=(SELECT id FROM users WHERE tg_id=:tg LIMIT 1) ORDER BY id",
            False,
        ),
        "events": ("SELECT * FROM events WHERE tg_id=:tg ORDER BY id", False),
        "analytics": (
            "SELECT * FROM analytics_events WHERE tg_id=:tg ORDER BY id",
            False,
        ),
    }
...
async def dp_wipe_ask(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    txt = (
        "🧹 Удалить данные: записи, события и аналитику.\nАккаунт останется."
        if not lang.startswith(("uk", "en"))
        else (
            "🧹 Видалити дані: записи, події та аналітику.\nАкаунт залишиться."
            if lang.startswith("uk")
            else "🧹 Delete data: journal, events and analytics.\nAccount will stay."
        )
    )
...
async def dp_delete_ask(call: CallbackQuery, session: AsyncSession) -> None:
    lang = await _get_lang(session, call.from_user.id)
    txt = (
        "❌ Удалить аккаунт навсегда. Это необратимо."
        if not lang.startswith(("uk", "en"))
        else (
            "❌ Видалити акаунт назавжди. Це незворотно."
            if lang.startswith("uk")
            else "❌ Delete account forever. This is irreversible."
        )
    )
...
async def dp_wipe_go(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    await _wipe_data(session, tg_id)
        ...
        "✅ Данные удалены."
        if not lang.startswith(("uk", "en"))
        else ("✅ Дані видалено." if lang.startswith("uk") else "✅ Data deleted.")
    )
...
async def dp_delete_go(call: CallbackQuery, session: AsyncSession) -> None:
    ...
    await _delete_account(session, tg_id)
        ...
        "✅ Аккаунт удалён."
        if not lang.startswith(("uk", "en"))
        else ("✅ Акаунт видалено." if lang.startswith("uk") else "✅ Account deleted.")
    )
    await call.message.answer(done)
    await call.answer()
